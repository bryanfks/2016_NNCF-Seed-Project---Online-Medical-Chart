<?PHP
	...
	if(!empty($seid) && !empty($sepwd)){
		...
		$chinapatient_sql ="UPDATE `PHpatient` SET `remark` = '".$remark."' WHERE `NCFMedicalNum` = '".$NCFMedicalNum."' LIMIT 1";
		
		
		
		$selChinaPatientRecord = "SELECT * FROM `PHrecord` WHERE NCFMedicalNum='".$NCFMedicalNum."'";
		...
		$chinapatientrecord_sql = "UPDATE `PHrecord` SET `ncfCheck` = '".$ncfCheck."',`ncfAllMedicaid` = '".$ncfAllMedicaid."', `ncfAllTransport` = '".$ncfAllTransport."', `ncfAllTotal` = '".$ncfAllTotal."', `ncfAllRemark` = '".$ncfAllRemark."' WHERE `NCFMedicalNum` = '".$NCFMedicalNum."' LIMIT 1";	  
		...
		//	echo "location.href='philippinesList.php'";
			echo "//-->";
			echo " </Script>";
			echo "</head>";
			echo "</html>";
