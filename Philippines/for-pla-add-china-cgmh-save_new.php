...
$descriptionCaseFamily = $_POST["descriptionCaseFamily"];
		...
		$pat_NCFID_path = "/var/www/vhosts/seed-nncf.org/httpdocs/Philippines/photo/patientrecord/".$NCFID; 		// 设定病患照片路径
		if(is_dir($pat_NCFID_path)){																													// 判断上列 病患照片路径 是否已存在
			...
			$oldmask = umask(0);
			mkdir("/var/www/vhosts/seed-nncf.org/httpdocs/Philippines/photo/patientrecord/".$NCFID, 0777);			// create new image fold by new member fold
			$img_path_patient = "/var/www/vhosts/seed-nncf.org/httpdocs/Philippines/photo/patientrecord/".$NCFID."/";		// photo on server path.
			umask($oldmask);
		}
		...
		$pat_NCFMedicalNum_path = "/var/www/vhosts/seed-nncf.org/httpdocs/Philippines/photo/medicalrecord/".$NCFMedicalNum; 		// 设定病患照片路径
