<?
	...
	if( (!empty($seid)) && (!empty($sepwd)) ){
		...
		if((!empty($srhValue))){
			$selChinaPatientinfo = "SELECT * FROM `PHpatient` WHERE `name` LIKE '".$srhValue."' OR `NCFID` LIKE '".$srhValue."' OR `MedicalRecordNumber` LIKE '".$srhValue."'"; 
			//屬廣泛搜尋法 ％：表示分配字元
...
<SCRIPT LANGUAGE="javascript">
	<!--
		...
		function fetch_Data(num){
			location.href = "for-pla-add-china-cgmh_new.php?cpi="+num+"&srhType=<?PHP echo $srhType; ?>&srhValue=<? echo $srhValue; ?>";
		}
		function addData(){
