<?PHP
	...
	if(!empty($seid) && !empty($sepwd)){
		...
		$qMaxNCFID = mysql_query("SELECT MAX(serialcode) FROM `patient-vn-record` WHERE NCFID='".$NCFID."'");
		...
		$pat_NCFID_path = "/var/www/vhosts/seed-nncf.org/httpdocs/Vietnam/photo/patientrecord/".$NCFID; 		// 設定病患照片路徑
		if(is_dir($pat_NCFID_path)){																													// 判斷上列 病患照片路徑 是否已存在
			$img_path_patient = "/var/www/vhosts/seed-nncf.org/httpdocs/Vietnam/photo/patientrecord/".$NCFID."/";		// photo on server path.
		} else {
			$oldmask = umask(0);
