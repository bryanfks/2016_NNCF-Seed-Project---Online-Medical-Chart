<?PHP
	...
	if(!empty($seid) && !empty($sepwd)){
		...
		//$sel_max = "SELECT MAX(num) FROM PKpatient";
		$sel_max = "SELECT MAX(NCFID) AS NCFID FROM PKpatient";
		$query = mysql_query($sel_max);
		$result = mysql_fetch_array($query);
		//$maxNum = $result['MAX(num)'];	//Get MAX num from China patient's num.
		
		//$maxData = "SELECT * FROM `PKpatient` WHERE num=".$maxNum."";
		...
		if(!empty($maxNums)){	// If $max !empty than split no
			...
			if($no_date == $date){
				...
				$NCFNumX = "PK".$date.$flow_no;
			}else{
				$NCFNumX = "PK".$date."0001";	
			}
		}else{
			$NCFNumX = "PK".$date."0001";
		}
	//echo $NCFNumX."<br>";
		...
		//$recordData = "SELECT MAX(NCFMedicalNum) FROM chinapatientrecord WHERE NCFID='".$NCFNumX."'";
		$recordData = "SELECT MAX(NCFMedicalNum) FROM PKpatientrecord WHERE NCFID='".$NCFNumX."'";
...
<body>
<form name="china_medical" enctype="multipart/form-data" method="post" action="for-pla-add-PK-cgmh-save.php">
<div align="center">
                        ...
                        <b style='mso-bidi-font-weight:normal'><span lang=EN-US>
        					<font size="5" face="Times New Roman">
        						<select size="1" name="school">
									...
									<OPTION value="CLAPP">CLAPP Hospital</OPTION>
