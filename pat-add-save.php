...
// -----------------------------------------------------------------------------		
			...
			$sel_flownum = "SELECT MAX(flownum) FROM patient WHERE nationality='".$nationality."' AND nowyears='".$get_time['year']."'";
			...
			//echo "Flow Number: ".$flowcode."<br/>";
			
			
			$sel_flownums = "SELECT * FROM patient WHERE flownum='".$flowcode."'";
			
			//echo $sel_flownums."<br/>";
			
			$query_flownums = mysql_query($sel_flownums);
			$flownDatecodes = mysql_fetch_array($query_flownums);
			$fds = $flownDatecodes["datecode"];
			$dateCodes = substr($fds,-4,4);
			// 上列步驟：1. 先取得最大流水編號（flownum），2. 從最大流水編號反搜datecode的年份碼。
			
			//echo "dateCodes Value: ".$dateCodes." Now year: ".$get_time['year']."<br/>";
			// 先取得最大病患流水編號的 datecode
...
// -----------------------------------------------------------------------------
		...
		$add_data = "INSERT INTO `patient` ( `num` , `patientid` , `firstname` , `lastname` , `nationality` , `gendered` , `birthday` , `birthmonth` , `birthyears` , `address` , `city` , `state` , `zipcode` , `country` , `telcountry` , `telarea` , `tel` , `fax` , `mobile` , `notes` , `addtime` , `chgtime` , `flownum` , `datecode` , `nowyears`)";
		$add_data .= "VALUES ('', '".$patid."', '".$firstname."', '".$lastname."', '".$nationality."', '".$gendered."', '".$birthday."', '".$birthmonth."', '".$birthyears."', '".$address."', '".$city."', '".$state."', '".$zipcode."', '".$country."', '".$telcountry."', '".$telarea."', '".$tel."', '".$fax."', '".$mobile."', '".$note."', '".$addtime."', '".$chgtime."' , '".$flownum."','".$dates."','".$get_time['year']."')";
