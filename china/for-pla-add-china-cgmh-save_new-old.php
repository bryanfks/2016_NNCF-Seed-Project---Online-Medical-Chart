...
$descriptionCaseFamily = $_POST["descriptionCaseFamily"];
		...
		$pat_NCFID_path = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFID; 		// 设定病患照片路径
		if(is_dir($pat_NCFID_path)){																													// 判断上列 病患照片路径 是否已存在
			$img_path_patient = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFID."/";		// photo on server path.
		} else {
			$oldmask = umask(0);
			mkdir("/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFID, 0777);			// create new image fold by new member fold
			$img_path_patient = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFID."/";		// photo on server path.
			umask($oldmask);
		}
		...
		if(!empty($filename)){
			...
			$picpaths = "http://www.seed-nncf.org/china/photo/patientrecord/".$NCFID."/".$filename;			// picture path for show picture on page.
...
*/		
		...
		// 指：粘贴其他资料图片
		$pat_NCFMedical_other = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFMedicalNum; 		// 设定病患照片路径
		if(is_dir($pat_NCFMedical_other)){																													// 判断上列 病患照片路径 是否已存在
			$img_path_patient_other = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFMedicalNum."/";		// photo on server path.
		} else {
			$oldmask = umask(0);
			mkdir("/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFMedicalNum, 0777);			// create new image fold by new member fold
			$img_path_patient_other = "/home/customer/seed-nncf.org/www/china/photo/patientrecord/".$NCFMedicalNum."/";		// photo on server path.
			umask($oldmask);
		}
		// 复制上传的图片到已病例号码所建立的资料夹内
